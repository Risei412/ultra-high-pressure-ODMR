...
fig, ax = plt.subplots(figsize=(8.4, 4.3) if ts.LEAN else (11.0, 5.6))

# --- forbidden region: one-photon ionisation out of 3A2 at 120 GPa ---------
ax.axvspan(lam[0], ed120, color=ts.CRIT, alpha=0.10, lw=0)
ax.axvline(ed120, color=ts.CRIT, lw=2.0)
ax.axvline(ed0, color=ts.CRIT, lw=1.4, ls=':')
...
ax.annotate('', xy=(ed120, 0.42), xytext=(ed0, 0.42),
            arrowprops=dict(arrowstyle='-|>', color=ts.CRIT, lw=2.2,
                            shrinkA=0, shrinkB=0))
ax.text(0.5 * (ed0 + ed120), 0.50,
        ts.t(f'イオン化端  {ed0:.0f} → {ed120:.0f} nm',
           f'ionisation edge  {ed0:.0f} -> {ed120:.0f} nm'),
        color=ts.CRIT, ha='center', va='bottom', fontsize=13.5, weight='bold')
if ts.LEAN:
    # slide_style_v3.md S9: the figure is evidence, the slide is interpretation.
    # Two arrows and one region label; the cross-section ratio moves to the slide.
    ax.text(0.5 * (lam[0] + ed120) - 1.5, 1.9,
            ts.t('1 光子\nイオン化', '1-photon\nionisation'),
            color=ts.CRIT, fontsize=11, va='center', ha='center', rotation=90)
    for lam0, lab in ((532., '532 nm'), (473., '473 nm')):
        ax.axvline(lam0, color=ts.MUTED if lam0 == 532 else ts.ACCENT,
                   lw=1.6, ls='--', alpha=0.9)
        ax.text(lam0 + (4 if lam0 == 532 else -4), 3.44, lab,
                color=ts.MUTED if lam0 == 532 else ts.ACCENT, fontsize=13.5,
                ha='left' if lam0 == 532 else 'right', va='top')
    ...
    ax.text(ed0 + 3, 0.07, ts.t('463 nm (0 GPa)', '463 nm (0 GPa)'),
            color=ts.CRIT, fontsize=10.5, ha='left')
...
plt.savefig(f'talk_three_shifts{ts.SUF}.png')
print(f'saved talk_three_shifts{ts.SUF}.png ; peak {pk0:.0f} -> {pk120:.0f} nm ; '
