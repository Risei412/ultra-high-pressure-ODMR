...
if ts.LEAN:      # one support field, not two team-coloured half-planes
    ax.axhspan(0.2, 1.0, color=ts.BAND, lw=0)
else:
...
ax.axvline(xo, color=ts.CRIT, lw=1.2, ls=':')
ax.plot(xo, 1.0, 'o', color=ts.CRIT, ms=11, mec='white', mew=2, zorder=6)
ax.annotate(ts.t(f'符号が変わる  {xo:.0f} GPa', f'sign change  {xo:.0f} GPa'),
            xy=(xo, 1.0), xytext=(xo + 8, 0.47), color=ts.CRIT, fontsize=14,
            weight='bold',
            arrowprops=dict(arrowstyle='->', color=ts.CRIT, lw=1.6))

# the published null result the model has to reproduce
ax.plot(50., ratio(50.), 's', color=ts.W532, ms=12, mec='white', mew=1.8,
        zorder=6)
ax.annotate(ts.t('50 GPa 実測: 優位なし [Bha22]' if ts.LEAN else
                 '50 GPa の実測:\n「青の明確な優位なし」\n[Bhattacharyya 2022]',
                 'measured at 50 GPa: no advantage [Bha22]' if ts.LEAN else
                 'measured at 50 GPa:\n"no distinct advantage"\n[Bhattacharyya 2022]'),
            xy=(50., ratio(50.)), xytext=(19., 1.35), color=ts.W532,
            fontsize=12.5,
            arrowprops=dict(arrowstyle='->', color=ts.W532, lw=1.5))

if not ts.LEAN:      # the axis already carries which side wins
    # the target
...
plt.savefig(f'threshold_green_blue{ts.SUF}.png')
print(f'saved threshold_green_blue{ts.SUF}.png ; crossover {xo:.1f} GPa ; '
