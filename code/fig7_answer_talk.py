...
_args = ts.positional()
P = float(_args[0]) if _args else 120.0
Pi = int(round(P))
...
fig, ax = plt.subplots(figsize=(8.4, 4.4) if ts.LEAN else (11.0, 5.8))

ax.axvspan(w_lo, w_hi, color=ts.BAND, alpha=0.85, lw=0, zorder=0)
ax.axhline(1.05, color=ts.ACCENT, lw=1.0, ls=':', alpha=0.8, zorder=1)

ax.fill_between(lam, bL, bH, color=ts.BAND if ts.LEAN else ts.ACCENT,
                alpha=0.9 if ts.LEAN else 0.16, lw=0, zorder=2)
...
PLACE = {405.: (ts.W405, ts.t('イオン化端', 'ionisation edge'), 407., 3.95, 'left'),
         473.: (ts.OP, ts.t('市販 DPSS', 'commercial DPSS'),
                477. if ts.LEAN else 492., 1.95 if ts.LEAN else 2.62, 'left'),
...
for lam0, (col, note, tx, ty, ha) in PLACE.items():
    ...
    ax.text(tx, ty, f'{lam0:.0f} nm', color=col, fontsize=13.5, weight='bold',
            ha=ha, va='top')
    if not ts.LEAN:      # the penalties live in the slide table, not the plot
...
out = f'talk_answer_{Pi}GPa{ts.SUF}.png'
plt.savefig(out)
