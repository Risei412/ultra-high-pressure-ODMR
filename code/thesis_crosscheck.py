...
T2  Fig. 6.2.  405 nm against 532 nm in hydrostatically compressed
    ...
    17 GPa and 71.7 at 100 GPa, with the crossover at 72.0 GPa.  Four orders
...
def ratio(model, lam, pressure, reference=REFERENCE_NM):
    """A(lam)/A(reference) at one pressure."""
    return absorption(model, lam, pressure) / absorption(model, reference,
                                                         pressure)
...
def report():
    ...
    print('      they compressed to ~70 GPa and report no change above '
          '~50 GPa;\n      the kernel predicts 1.9x by 70 GPa, so this is '
