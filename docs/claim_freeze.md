...
| 2 | 0:40–1:50 | **試料は 2 枚のダイヤモンドの先端にある**（DAC を知らない聴衆向け） | `assets/lit_dac_schematic.png` ＋ T_c–圧力の表 |
| 3 | 1:50–2:40 | **アンビルの内側の NV が局所の磁束排除を撮る** ＋ 感度が通貨 | `assets/lit_nv_in_anvil.png`, `assets/lit_meissner_map.png` |
...
スライド 2–3 の論理は `slides/intro_deck.html`（¶1–¶4）を圧縮したもの:
DAC とは何か → 狙う圧力帯 → 抵抗では決着しない → 探針をアンビルの内側に作る →
既に撮れている実測 → だから感度が通貨。
先行文献の図は `slides/assets/`（出所は同ディレクトリの README.md）。
