...
BOTTOM = {
    ...
    5: ['形を決める 3 つ以外は、**±50% 振っても λopt は動かない**。'],
...
NOTES = {
    ...
    6: '3:55–4:45  答え。窓の広さと 473 nm の位置。'
       ...
       '［想定質問］温度依存は? λopt は 4 K と 300 K で 0.6 nm しか動かない。'
       '温度はフランク–コンドン包絡にしか入らず、包絡は対称に広がるので'
       'ピークは動かない。むしろ冷やすと反ストークス側が消えて'
       '赤側の裾にいる 532 nm が更に落ち、緑とのペナルティ比は '
       '300 K の ×2.5 から 150 K で ×3.4、4 K で ×4.9 に開く。'
       '室温の ×2.5 は保守的な値である。',
    ...
    9: '6:50–7:35  理論を凍結し、次の一手に渡す。'
       '低温で測ることになるが、①②を冷やしてやり直す必要はない — '
       'λopt の温度依存は 0.6 nm で、常温で選んだ線が低温でも正しい。'
